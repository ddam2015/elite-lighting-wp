<?php
	...
	foreach( $curr_elements as $el ) {
		...
		else if ( in_array( $el, array( 'range', 'sort', 'instock', 'price', 'per_page', 'search' ) ) ) {
			$el_fil = false;
		}
	...
	$pf_termcount = WC_Prdctfltr::$settings['wc_settings_prdctfltr_termcount'];
					...
					<div class="prdctfltr_filter prdctfltr_attributes prdctfltr_<?php echo $curr_el; ?> <?php echo $curr_term_style; ?><?php echo $curr_term_multi; ?><?php echo $curr_term_customization; ?><?php echo $curr_term_adoptive; ?><?php echo $curr_term_relation; ?><?php echo $curr_term_expand; ?><?php echo $curr_term_subonly; ?>" data-filter="<?php echo $curr_fo['filter']; ?>" data-limit="<?php echo $curr_limit !== 0 ? $curr_limit-1 : '0';?>">
						...
						<?php
							...
							foreach ( $catalog_categories as $term ) {
								...
								if ( !in_array( $decode_slug, $active_filters[$curr_fo['filter']] ) ) {
									$active_filters[$curr_fo['filter']][] = $decode_slug;
								}
								...
								$term_count_real = $pf_termcount == 'deep' ? WC_Prdctfltr::published_term_count( $term->term_id, $curr_fo['filter'] ) : $term->count;

								if ( empty( $curr_fo['settings']['customization'] ) ) {

									$term_count = ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $term_count_real == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$term->slug]) && $output_terms[$curr_fo['filter']][$term->slug] != $term_count_real ? WC_Prdctfltr::get_term_count( $output_terms[$curr_fo['filter']][$term->slug], $term_count_real ) : ( isset($output_terms[$curr_fo['filter']]) && !empty($output_terms[$curr_fo['filter']]) && !isset($output_terms[$curr_fo['filter']][$term->slug]) ? 0 : $term_count_real ) ) . '</span>' );
								...
								if ( isset( $curr_fo['settings']['hierarchy'] ) && $curr_fo['settings']['hierarchy'] == 'yes' && !empty( $pf_children ) ) {
									...
									foreach( $pf_children as $sub ) {
										...
										$term_count_real = $pf_termcount == 'deep' ? WC_Prdctfltr::published_term_count( $sub->term_id, $curr_fo['filter'] ) : $sub->count;
										...
										if ( !empty($sub->children) ) {
											...
											foreach( $sub->children as $subsub ) {
												...
												$term_count_real = $pf_termcount == 'deep' ? WC_Prdctfltr::published_term_count( $subsub->term_id, $curr_fo['filter'] ) : $subsub->count;
												...
												if ( !empty($subsub->children) ) {
													...
													foreach( $subsub->children as $subsubsub ) {
														...
														$term_count_real = $pf_termcount == 'deep' ? WC_Prdctfltr::published_term_count( $subsubsub->term_id, $curr_fo['filter'] ) : $subsubsub->count;
														$pf_adoptive_class = '';
